...
$(document).ready(function() {
    ...
    function fetchData() {
        fetch(base_url + 'adminactivedomain/api')
            ...
            .then(data => {
                ...
                dataTable.draw(); // Redraw the DataTable with new data
            })
            .catch(error => {
                console.error('Error:', error);
            });
    ...
    $('#update-modal').on('show.bs.modal', function(event) {
        ...
        var active_id = button.data('active_id');
        var modal = $(this);
        
        $.ajax({
            url: base_url + 'adminactivedomain/get_domain/' + active_id,
            method: 'GET',
            dataType: 'json',
            success: function(data) {
                if (data.success) {
                    var domainData = data.data;
                    modal.find('.modal-body #u_active_id').val(domainData.active_id);
                    modal.find('.modal-body #u_url').val(domainData.url);
                    modal.find('.modal-body #u_comment').val(domainData.comments);
                    var tags = domainData.tags.split(';');
                    modal.find('.modal-body #u_tags').val(tags);
                }
            },
    ...
    $('#update-modal-form').submit(function(event) {
        ...
        $.ajax({
            url: base_url + 'adminactivedomain/update_modal',
            method: 'POST',
            data: formData,
            dataType: 'json',
    ...
    $('#refresh-button').click(fetchData);
});
</script>
